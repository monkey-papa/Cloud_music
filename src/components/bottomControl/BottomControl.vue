<template>
  <div>
    <div class="bottomControls" v-if="isShow">
      <lyrics-scrolls :lyric="lyric"></lyrics-scrolls>
    </div>
    <!-- 底部播放栏以及播放栏抽屉 -->
    <div class="bottomControl">
      ...
      <div class="center">
        <div class="buttons">
          <span @click="playType = playType == 'order' ? 'random' : 'order'">
            <i class="iconfont icon-xunhuan" title="顺序播放" v-if="playType == 'order'"></i>
            <i class="iconfont icon-suiji1" title="随机播放" v-else></i>
          </span>
      ...
      <el-drawer :visible.sync="drawer" :with-header="false" width="300">
        ...
        <div>
          <el-table class="my_table" :data="musicList" :cell-style="{ background: '#202023' }" style="width: 100%" :show-header="false" @row-click="clickRow" highlight-current-row lazy v-if="musicList.length > 0">
          ...
          <div v-else class="none">
            <span class="noList">还没有添加任何歌曲喔(*╹▽╹*)</span>
          </div>
        </div>
      </el-drawer>
    </div>
...
<script>
import { handleMusicTime, returnSecond, FormatLyric } from 'plugins/utils'
import LyricsScrolls from '@/components/lyricsScroll/LyricsScrolls.vue'
let lastSecond = 0
// 总时长的秒数
let durationNum = 0
// 保存当前音量
