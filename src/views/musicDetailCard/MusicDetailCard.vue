<template>
  ...
  <div class="musicDetailCard" :class="[isMusicDetailCardShow ? '' : 'hide', backgroundMode ? '' : 'glassCard']">
    ...
    <div class="musicDetailCardContainer" v-if="!cleanCard && musicInfo.name">
      ...
      <div class="bottom" v-loading="isCommentLoading" element-loading-background="rgba(255, 255, 255, 0)">
        <comment :comments="hotComments" :commentType="'music'" :commentId="$store.state.musicId + ''" :musicTitle="musicInfo.name" @getComment="getMusicComment($store.state.musicId)" class="commentComponent" v-if="currentCommentPage == 1"><div slot="title">热门评论</div></comment>
        <comment :comments="comment.comments" :commentType="'music'" :commentId="$store.state.musicId + ''" :musicTitle="musicInfo.name" @getComment="getMusicComment($store.state.musicId)" class="commentComponent"
          ><div slot="title">最新评论({{ comment.total }})</div></comment
        >
        <!-- 评论分页 -->
        <div class="page" v-show="comment.comments && comment.comments[0]">
          <el-pagination background layout="prev, pager, next" :total="comment.total" small :page-size="20" :current-page="currentCommentPage" @current-change="commentPageChange"> </el-pagination>
        </div>
      </div>
...
export default {
  ...
  methods: {
    // 请求
    async getLyric(id) {
      this.lyric = await FormatLyric(id)
    },
    // 请求评论数据
    ...
    changeBackground() {
      if (navigator.userAgent.indexOf('Firefox') != -1) {
        // console.log("ff");
        this.$message.info('火狐暂不支持玻璃背景功能，请使用Chrome或Edge浏览器')
        return
      }
...
.musicDetailCard {
  ...
  background-image: linear-gradient(to bottom, #e3e2e3, #000000);
}
